...
namespace Dresser.Services.Ipc {
	internal class AllaganToolsService : IDisposable {
		...
		public IEnumerable<CriticalCommonLib.Models.InventoryItem> GetCharacterItems(ulong characterId)
			=> GetCharacterItemsSerialized(characterId).Select(Models.InventoryItem.FromNumeric);
		public IEnumerable<Models.InventoryItem> GetItems(ulong characterId)
			=> GetCharacterItems(characterId).Select(Models.InventoryItem.FromCritical);
		public Dictionary<ulong, IEnumerable<Models.InventoryItem>> GetItemsLocalCharsRetainers(bool includeActiveCharacter = false)
